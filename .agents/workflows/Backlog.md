...
- **Authorization and responsibility are separate.** Authorization is the wizard-issued ceiling; the per-action responsibility map assigns preparation, readiness, implementation/evidence, publication, landing, status/close, replacement, and cleanup. Git-visible actions never exceed the latest effective authorization revision or act outside the assigned owner. The wizard enumerates the concrete actions each path implies (informed consent), and merges are bounded to the triaged `issue_scope`. Destructive git is prohibited and is **not** grantable via any scope.
...
2. **delegation topology and approval mode** — safely call-test worktree allocation and first-level Loop dispatch topology (clean up probe artifacts), then call-test actual test, implementation, review, and refactor roles; record each `available|unavailable|unproven` with evidence/topology/timestamp, mutation approval mode, and concurrent-orchestrator risk.
...
Persist keyed oracle budgets before use: readiness allows one substantive verdict/two transport attempts per contract tuple; clarity/conflict decisions use separate keys; classification allows one/two per stable signature. Preserve counters across restart/replacement and distinguish key exhaustion from lineage exhaustion. Deterministic checks consume no oracle budget, and Backlog never performs Loop's independent verdict.

Then ask everything once via `ask_user`, then proceed unattended:

1. **Confirm/adjust the triaged order** (show the work queue).
2. **Exempt-close batch and concurrency** — confirm one-shot close of the `wontfix`/`duplicate` items (or remove any), and collect `concurrent_orchestration: none|yes|unknown` (default `unknown`).
3. **Completion and contract authority** — collect `contract_maintenance: deny|workflow-only` (default `workflow-only`), `contract_publication_scope: with-issue-only|branch+pr|branch+pr+merge` (default `with-issue-only`), item-specific `conformance_acceptances` (default none), explicit per-issue `degraded_ok` (default false), and completion path. Show delegation and Loop/worktree capability summaries.
...
**Persist wizard answers and authorization revision** (`revision`, `issuer`, `issued_at`, `effective_at`, `git_scope`, `contract_maintenance`, `contract_publication_scope`, `doc_edits`, `issue_scope`, `conformance_acceptances`, `stop`, `granted_at`) plus a separate per-action responsibility map into `docs/progress/backlog-<run>.md` at Phase 2 — this is what makes the run resumable without re-asking. State the invariants in the wizard prompt so the user sees them.

## Phase 3 — Per-issue pipeline

Maintain a flight set of **≤3** Loop subagents — **filled by default**, not drained one-at-a-time. On the first dispatch, fill every free slot with an independent issue (step 2 conflict-risk gate); as each reports `merge_ready` → run 3d/3e/3f, then dispatch the next independent queued issue into the freed slot. For each queued work item:

### 3a. Contract preparation, identity, and clarity gate

Before external preparation, run the §3c orphan/collision sweep, wait for dependencies, refresh default with fetch + `--ff-only`, then allocate the issue branch/worktree once directly from refreshed default, and record its base SHA. Contract workflows commit there; retain/rebind that worktree for Loop so the contract commit is reachable without stacking or copying.

- Backlog owns external contract preparation; Loop never creates or repairs its inputs. If both Spec and Deep Plan are linked, current, and non-blocking, resolve immutable identity and continue to 3b.
- If either artifact is missing, stale, or reconciliation-blocking, run a **two-gate clarity check** before generating only the required artifact(s):
  ...
  - **Both pass** → invoke **Spec** when absent, stale, or explicitly reconciliation-blocking; then reassess the Plan against the resulting/current Spec and invoke RPCE core **Deep Plan** when absent, stale, or reconciliation-blocking. Link changed artifacts via track-work. Record request, starting commit, target paths, maintenance outcome, exact commit range, and dirty paths. Only `committed_complete` re-enters deterministic readiness; `committed_partial|dirty_partial` preserves work, consumes no readiness verdict for the incomplete tuple, and records an exact resume instruction.
  - **Either fails** → set `status:blocked` via track-work with the reason; skip the issue.
- **Oracle-down policy (C1).** If the Phase-2 pre-flight found oracle unreachable: issues **with** an existing spec+plan proceed with deterministic gates authoritative only when the persisted revision explicitly grants `degraded_ok` for that issue (carry the value and record the degraded verdict); otherwise block; issues **needing spec generation** are blocked — the oracle confirm is load-bearing and specs are never generated ungated. The orchestrator owns all scope/ambiguity decisions for degraded issues; subagents may use `code-review` for *diffs* but never as a scope-decision oracle.

### 3b. Readiness gate

Resolve and persist `contract_commit`; Spec path + blob SHA; Plan path + blob SHA; tuple key; `base_sha`; and separate `implementation_base_sha`. Require tracked clean paths, both blobs at the same commit, worktree/blob equality, and contract-commit reachability from the issue branch. Run **spec-plan-readiness** against that exact tuple; stale tuple verdicts cannot dispatch. Blocked → set `status:blocked`, skip.

### 3c. Allocate a unique worktree and dispatch Loop

1. **Branch:** `backlog/<track-work-id>-<slug>`. Reuse the issue branch/worktree allocated in §3a after confirming its contract commit and refreshed-default base. If no preparation was needed, allocate it now using the same dependency-wait, fetch, `--ff-only`, and direct-from-default procedure. Never start from another issue branch.
...
3. **Pre-dispatch sweep + collision check.** Before the single issue-worktree allocation in §3a/§3c, reconcile orphans from prior interrupted runs so the new dispatch never collides with a stale branch/worktree:
   ...
   - **Branch exists** (`git show-ref --verify refs/heads/<branch>`):
     - with a **live bound session whose persisted state is `paused`** → **resume** it (`agent_run op=steer`). Any other state requires a new epoch. Do not create a second worktree.
     - with **no live session** but a worktree present → preserve it. Rebind only after exclusive-ownership proof and predecessor audit; otherwise record `restart_required`. Never suffix around executed, dirty, or ambiguous predecessor work.
     - with **no session and no worktree** → **verify clean half-provisioning before reuse**: `git log <base>..<branch>` empty, or `git merge-base --is-ancestor <branch> <base>`. At base, or at the clean ledger-recorded contract HEAD from completed preparation with zero Loop turns, → safe to reuse **only if no dead/skipped session remains bound to it**. Has commits, executed/dirty/ambiguous predecessor evidence, or a remaining binding → preserve original identity and record `replacement_required|restart_required`; never suffix. A suffix is allowed only for proven-clean half-provisioning with no contract preparation commits, executed epoch, or binding.
4. **Record the dispatch attempt, then dispatch.** First persist the attempt number, exact deterministic `session_name`, branch, and pipeline phase `dispatching` in the ledger; that write must complete before provisioning begins. Before dispatch, revalidate the Phase-2 required-role capability record and persist any change. Dispatch only if all required roles are `available`; never infer capability or perform Loop work inline. Then dispatch a fresh Loop subagent bound to a dedicated worktree. **This `op=start` is the only call in its tool batch** (Dispatch-isolation invariant) — the completed ledger write is a prior call, and no `gh`/board/status mutations, verify/test commands, or `cleanup_sessions` share the start's batch:
...
{"tool":"agent_run","args":{
  ...
  "workflow_name":"Loop",
  "worktree":"<existing issue worktree id/path>",
  "session_name":"Backlog <id>: <title>",
  "detach":true,
  "message":"progress_path=<path>; lineage=<id>; epoch=<id>; maintenance_outcome=<outcome>; predecessor_handoff=<none|record>; delegation_capabilities=<records>; oracle_budgets=<keys+counters>. Initialize the owned progress path before full contract reads. Then read spec (<path>, blob <sha>) and Deep Plan (<path>, blob <sha>) for contract_commit <sha>; tuple_key=<key>; base_sha=<sha>; implementation_base_sha=<sha>. You are already bound to a dedicated worktree on branch <branch> — do NOT create or switch worktrees; record the base SHA and proceed. Run the Loop workflow to completion for issue <id>, in ORCHESTRATED mode (escalation principal = this orchestrator; never ask the end-user — return terminal blocked/handoff reports to me; wait for steer only after persisting live `paused`). Brief contract: authorization={revision: <n>, issuer: backlog, issued_at: <time>, effective_at: <time>, git_scope: <path>, contract_maintenance: <scope>, contract_publication_scope: <scope>, doc_edits: <bool>, issue_scope: <ids>, conformance_acceptances: <items>, stop: <bool>, granted_at: <time>}; responsibility={preparation: backlog, deterministic_readiness: backlog, independent_readiness: loop, implementation_evidence: loop, publication: backlog, landing: backlog, status_close: backlog, replacement: backlog, cleanup: backlog}; toolchain={<abs test/build cmds + env from the repo>}; oracle={status: <up|down>, degraded_ok: <true|false>}; user_testing={tool: <name|none>, data_location: <throwaway dir — never the user's real data>}; amendments=[<any pending>]; siblings=[<other in-flight issues/areas>]; constraints={<repo hard-rules pulled from CLAUDE.md/AGENTS.md — inject verbatim, never hardcode repo specifics here>}. Do NOT change the issue's status or labels (the outer Backlog layer owns that). Report the closeout-evidence contract: scenario IDs covered, tests added + results, validation commands + results, spec-conformance matrix path, amendments applied/declined (+ grep/diff evidence), user-testing result (tool + data location + result|blocked-reason, if UI), and review-finding signatures."
}}
```
...
5. **Liveness-confirm and reconcile every dispatch attempt (do not skip).** Each invocation of `start` increments the persisted attempt counter. Treat the attempt as `dispatching` until a provider is positively confirmed; do not add it to the flight set, free its slot, mutate issue status, or start another attempt for that issue before reconciliation. Add `session_id`/`worktree_id` to the pre-start ledger entry when discovered.
   ...
   - discovered or returned `idle` **and** 0 assistant turns, or an expired handle → **half-provisioned**. **Do not `steer`** — `steer` injects a message but **cannot launch a provider**. Recover by: `agent_manage op=cleanup_sessions` the dead session → `manage_worktree op=unbind` + `git worktree remove <path>` the orphan worktree. If cleanup succeeds, reuse/rebind the same branch if verified clean and unbound at either base or the ledger-recorded contract HEAD; a non-base retry additionally requires exact preparation-range/tuple verification and proof of zero Loop turns; when cleanup succeeds and reuse proof passes, re-dispatch once with an isolated `start` and rerun reconciliation. If cleanup skips or the dead session remains visible/bound, quarantine/preserve the original branch, forbid suffixing, record `restart_required`, and stop starts for that issue until the restart sweep confirms the binding is gone.
    matching branch/worktree but no matching session → **partially provisioned**. Verify provenance and ownership. Unbind/remove/reuse only for proven-clean, unexecuted, at-base half-provisioning; any non-base, dirty, executed, or ambiguous state is preserved on its original identity and routes to audited replacement or `restart_required`, never a suffix.
...
1. **Evidence read.** Independently inspect the exact recorded contract-preparation range and implementation range as separate diffs, confirming expected paths/commits and no cross-range contamination. Require the disclosed progress path, lineage, epoch, and durable `initialization_complete`; verify it chronologically precedes accepted implementation changes, oracle calls, delegations, tests, and production writes. Confirm the exact contract tuple and lifecycle state/phase. Then spot-check the diff on at least one load-bearing file (`read_file` / `git`), and **grep for each amendment's requested change** (feeds amendment follow-up, below). Confirm conformance status is `passed`, or `blocked_unaccepted` otherwise. Every Diverged/Not-built item needs `{id, rationale, principal, auth_revision, timestamp}` preauthorized by the responsible principal; generation/oracle cannot accept. Confirm no unaccepted P0/P1 review findings remain; confirm `user-testing` ran for any UI change.
...
Insufficient evidence → steer the same session only if it is live and its persisted state is explicitly `paused`; otherwise preserve evidence and start a new epoch for focused follow-up. Do not close.

**Amendment follow-up (D8).** If V1's grep shows an amendment's requested change is absent (ignored): steer once when the epoch is live `paused`, or file a follow-up issue via track-work. A follow-up never creates acceptance: conformance remains `blocked_unaccepted` unless the latest authorization already contains item-specific principal/revision acceptance. Never ask the user; never steer twice for the same amendment.
...
**Backlog lands as assigned owner (R4).** Loop reports `merge_ready` and performs no push/PR/merge. Backlog rechecks the latest effective authorization, independent verification, finalized artifacts, and conformance `passed`, then mechanically performs assigned publication/landing and close metadata. A Loop that stops before `merge_ready` is incomplete and returns to verification/recovery; Backlog does not infer readiness from commits. Once `merge_ready` is independently verified, Backlog completes only the latest-authorized publication/landing path and records it.

If the close policy is "present evidence only," skip the close and hand the verified evidence to the user.

### 3f. Cleanup + ledger

- **Ledger (F1).** Record every issue with lineage/epoch, progress path, `session_id`, worktree/branch/HEAD/manifest, full contract identity, contract-preparation and implementation ranges, `base_sha`, `implementation_base_sha`, authorization revision + responsibility, delegation capabilities, oracle keys/counters, maintenance outcome, conformance status/acceptances, predecessor/replacement handoff, lifecycle `state` and separate pipeline `phase`, amendments, and divergences. Resume must restore these fields and reject stale reports/revisions/counters.
...
- `cleanup_sessions` may **skip** (it cannot force-delete) — tolerate that: mark the row `cleanup: skipped`, quarantine and preserve any branch still bound to that session, and record `restart_required`; never re-steer a dead session or assume worktree removal released its branch binding.
...
## Lifecycle, replacement, amendments, and publication

Use the Backlog Spec Proposed Surface as canonical. Only a live `paused` epoch is steerable and owns its retained worktree. `terminated`, `handed_off_outside_loop`, `replacement_required`, `blocked`, and `completed` require a new epoch; handoff/block is never completion. Rollup and cleanup use `state` plus `phase`, never overload one field.

Dead/dirty predecessor recovery preserves exact worktree, branch, HEAD, manifest, and epoch. Existing-path replacement is disabled unless a call-based probe proves exclusive worktree ownership; otherwise record `restart_required`. A replacement audits candidate WIP and re-establishes meaningful red from `implementation_base_sha`; ambiguous chronology blocks. Backlog performs no tests, code, refactor, or review during recovery.

Amendments produce monotonic authorization revisions without another wizard. Prospective expansion applies only after its effective point; reduction/stop applies before the next action. Reject stale revisions and defer ambiguous or scope-expanding amendments. Responsibility remains separate and is not expanded by authorization.

Issue branches wait for dependencies while default refreshes non-destructively, then start directly from refreshed default; never stack issue branches. Publication is mechanical and requires clean committed finalized work, independent verification, conformance `passed`, and a latest-authorization recheck. Contract-only ranges remain visible as `local|pushed|in_pr|landed`; publication requires `contract_publication_scope`. General `doc_edits` authorizes neither contract maintenance nor publication and does not control mandatory progress/conformance artifacts.
...
- per-issue outcome, responsibility, lifecycle state/phase, dirty/recovery disposition, worktree/branch, exact contract and implementation ranges, contract-only publication disposition, PR/commit ref, delegation capability gaps, oracle key/lineage exhaustion, and conformance status/acceptances;
...
| Dispatch Loop (worktree-isolated) | `agent_run op=start workflow_name=Loop worktree=<existing issue worktree> detach=true` — **alone in its tool batch**; fill the flight set back-to-back on first dispatch |
...
- 🚫 Asking orchestrated Loop to publish or land. Loop stops at `merge_ready`; Backlog, as assigned owner, rechecks authorization and verification before publication/landing.
- 🚫 Treating authorization as responsibility, accepting stale revisions, or replacing a dirty predecessor without exclusive-ownership proof and audited red evidence.
