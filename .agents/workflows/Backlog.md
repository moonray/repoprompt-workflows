...
- `cleanup_sessions` may **skip** (it cannot force-delete) — tolerate that: mark the row `cleanup: skipped` and **route around** the orphaned session, never re-steer a dead session.
